...
def draw_player(game, x, y, wep, rot):
    image = pg.transform.rotate(game.player_images[wep], rot)
    rect = image.get_rect()

    rect.center = (x, y)

    game.screen.blit(image, game.camera.apply_rect(rect))

class OPlayer(pg.sprite.Sprite):
    def __init__(self, game, x, y, wep, rot, id, bullets):
    ...
    def shoot(self):
        ...
        if now - self.last_shot > WEAPONS[self.weapon]['rate']:
            ...
            for a in range(WEAPONS[self.weapon]['bullet_count']):
                ...
                dir = dir.rotate(spread)
                Bullet(self.game, pos, (dir.x, dir.y), self.rot, self.weapon)
                try:
...
class Enemy(pg.sprite.Sprite):
    def __init__(self, game, x, y, weapon, id, last_known=None, move=True):
        ...
        if last_known is None or self.can_move is False:
            self.moving = False
...
class Bullet(pg.sprite.Sprite):
    def __init__(self, game, pos, dir, angle, weapon, o=False):
        self._layer = BULLET_LAYER

        if o is False:
            self.groups = game.all_sprites, game.bullets, game.bulletsO
        else:
        ...
        self.vel = self.dir * WEAPONS[game.player.weapon]['bullet_speed'] * random.uniform(0.9, 1.1)
